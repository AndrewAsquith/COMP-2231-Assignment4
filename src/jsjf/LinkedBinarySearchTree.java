...
public class LinkedBinarySearchTree<T> extends LinkedBinaryTree<T> implements BinarySearchTreeADT<T> {
	...
	public T removeMax() throws EmptyCollectionException {
		...
		if (isEmpty()) {
			...
			// if the root.right is null root is the max
			if (root.right == null) {
				// grab the element to return and make the left child the new
				// root
				result = root.element;
				root = root.left;
			} else {
				...
				while (current.right != null) {
					// find the farthest right leaft
					parent = current;
					current = current.right;
				}
				// get the element to return
				result = current.element;
				// set the parent's right child to the removed node's left child
	...
	public T findMin() throws EmptyCollectionException {
		...
		if (isEmpty()) {
			...
			// if root's left child is null root is smallest
			if (root.left == null) {
				result = root.element;
			} else {
				...
				BinaryTreeNode<T> current = root.left;
				while (current.left != null) {
					// go left until reaching the farthest left leaf
					current = current.left;
				}
	...
	public T findMax() throws EmptyCollectionException {
		...
		if (isEmpty()) {
			...
			// if the root right child is null, root is max
			if (root.right == null) {
				result = root.element;
			} else {
				...
				BinaryTreeNode<T> current = root.right;
				while (current.right != null) {
					// go to the farthest right leaf
					current = current.right;
				}
				// farthest right will be max
				result = current.element;
			}
		}
		// return the element that was found
	 ...
	 * binary tree. Throws a NoSuchElementException if the specified target
	 * element is not found in this tree.
	 *
	 * @param targetElement
	 *            the element being sough in the binary tree
	 * @throws ElementNotFoundException
	 *             if the target element is not found
	 */
	public T find(T targetElement) throws ElementNotFoundException {
		...
		//search for the element starting at the root
		BinaryTreeNode<T> result = findNode(targetElement, root);
		
		//if it was not found result will be null, throw exception
		if (result == null) {
			throw new ElementNotFoundException("LinkedBinarySearchTree");
		} 
	...
	public LinkedBinarySearchTree<T> getLeft() {
		...
		//if the left child is null, return null
    	if (root.getLeft() == null) {
    		return null;
    	}   	
    	
    	
    	//(ab)use the protected properties to set the root 
    	//node of the new tree to the left child 
    	//instead of rebuilding the tree
    	//as the node already has the structure
	...
	public LinkedBinarySearchTree<T> getRight() {
    	...
    	// if the right child is null, return null
    	if (root.getRight() == null) {
    		return null;
    	}
    	
    	//(ab)use the protected properties to set the root 
    	//node of the new tree to the right child 
    	//instead of rebuilding the tree
    	//as the right child already has the structure
	 ...
	 * @param next
	 *            the tree node to begin searching on
	 */
	private BinaryTreeNode<T> findNode(T targetElement, BinaryTreeNode<T> next) {
		...
		//if the next node is the correct node, return it
		if (next.getElement().equals(targetElement)) {
			return next;
		}
		
		//otherwise, keep going down the tree, left child first
		BinaryTreeNode<T> childResult = findNode(targetElement, next.getLeft());
		
		//if the left child returned null, check the right
		if (childResult == null) {
			childResult = findNode(targetElement, next.getRight());
		}
		
		//return the result of searching the children
		return childResult;
